...
class Mesh:
    ...
    def _initialize_connectivity_array(self):
        ...
        for k in range(self.n_y):
            for n in range(self.n_x):
                ...
                triangle_type_int = self.map_triangle_type_int(self._get_triangle_type(k, n))                              
                self._connectivity_array[2 * (k * self.n_x + n) + 1, 3] = triangle_type_int
    ...
    def map_triangle_type_int(self, type: int) -> TriangleType:
        inverse_map = {v: k for k, v in self._type_to_int_map}
        return inverse_map[type]
        
    def map_triangle_type_int(self, triangle_type: TriangleType) -> int:        
